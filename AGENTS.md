...
- [x] New claims entry (10 August 2026) on funding and building maintenance responsibility, using the primary Final Report PDF text (Chapter 1, paragraphs 22&ndash;23 and 35&ndash;37; Chapter 6, paragraphs 6.46&ndash;6.47, all TRP-001). Documented from the Commission's own words: Galway County Council owned Tuam and was legally responsible for its upkeep; the Sisters received no salary and funded staff only from the capitation payment; the Commission found no evidence of profiteering by religious orders generally; and a 1953 visiting-committee member records the Sisters trying (and failing) to get hot water installed, and the Sisters refusing an offer to take on full ownership of the home until it was properly upgraded. Written with full balance: the same report also finds conditions stayed "primitive" until 1961 closure, and attributes part of the delay in investment to the Sisters as well as to the council (an unwillingness on both sides to end the practice of keeping children at Tuam years longer than at other homes). Searched again for the primary-source text of the "careful and attentive" Litster quote above; still not located in the sections of the Final Report reviewed, so that quote remains flagged as pending independent verification
- [x] Added a plain, factual, symmetrically-worded note to `methodology.html` (10 August 2026): the individual Sisters who served at Tuam are deceased, so no first-hand reply is available to confirm or contest specific descriptions of their own conduct; the same absence also means no surviving account of conditions can be tested against a reply from the person it describes. Framed as a limitation of the evidence, not as a reason to discount any particular source
...
- [x] Simple page-view counter (22 August 2026): Microsoft Clarity removed after it failed to register Irish visits reliably. Replaced with a cookieless Cloudflare Worker counter that shows "Page views: N" in the site footer. Counts page loads (not unique people). Worker: `cloudflare/page-counter.js`, live at `https://tuam-page-counter.tuamrecordsproject.workers.dev`. Bot filtering added the same day: known crawler user-agents, missing Origin, and automated browsers are not counted
...
- [x] Worker deployed live at `https://tuam-corrections-relay.tuamrecordsproject.workers.dev`, `GITHUB_TOKEN` secret set (a repo-scoped, fine-grained GitHub PAT with Issues: Read and write), and `RELAY_URL` in `contact.html` filled in and tested end to end (11 August 2026): a real test submission created a labelled `correction` GitHub Issue, confirming the relay works, then that test issue was closed. Visitors submitting the form now never see GitHub or leave the site; the old GitHub-issue link only shows if `RELAY_URL` is ever emptied again.
- [x] Comment/"have your say" section: decision unchanged from the note above (skipped for spam/moderation/credibility reasons); not built.
